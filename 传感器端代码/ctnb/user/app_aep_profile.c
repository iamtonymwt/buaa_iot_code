...
//THREAD_INIT(led_process, "led Thread");
...
static unsigned int  reportPeriod = 10;
...
static void nb_timer_cb(void *args)
{
	...
	temperatute_humidity_report reportFlag;
	AepString      reportstr;
		
	reportFlag.temperature_data = ((float)((int)(BSP_status.temp*10)))/10;
	reportFlag.humidity_data = BSP_status.hump;
	reportstr = codeDataReportByIdentifierToStr("temperatute_humidity_report", &reportFlag);
	app_log(reportstr.str);
	
		//if (led_flag == 1 && flag == 1) {
...
void decode_aep_data(char *data)
{
	...
	if( result.code == AEP_CMD_SUCCESS )
	{
      ...
      ret = send_msg(rsp.str, rsp.len, SEND_MODE_CON);
		...
		else if( strcmp(result.serviceIdentifier, "led_control") == 0 )
		{
			led_control_ack ack;
			AepString rsp;
		  uint8_t value;
			...
			ack.control_int = 1;
			ack.taskId      = result.taskId;
			rsp = codeDataReportByIdentifierToStr("led_control_ack", &ack);
			ret = send_msg(rsp.str, rsp.len, SEND_MODE_CON);
			if( ret < 0 )
			{
				app_log("send led_control_ack err\r\n");
			}
			free(rsp.str);
		}
	}
...
void aep_thread_init(void)
{
	...
	//thread_os_register(&led_process, NULL);
